...
(() => {
  ...
  function transactionHtml(row) {
    ...
    const blackCashMovement = ["black_deposit", "black_withdrawal"].includes(row.operation_type);
    const dirtyMovement = row.money_type === "dirty" || row.account === "black" || blackCashMovement;
    return `<article class="compta-transaction"><span class="compta-transaction-icon">▣</span><div><strong>${esc(title)}</strong><small>${esc(detail)}</small></div><span class="compta-amount ${positive?"positive":"negative"}${dirtyMovement?" is-dirty-cash":""}">${positive?"+":"−"} ${money(row.amount)}</span></article>`;
  }
